...
export class LightweightLiveSyncSettingTab extends PluginSettingTab {
  ...
  private renderSetupSection(containerEl: SettingsContainer): void {
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        this.plugin.settings.configured
          ...
          : "Prepare a server-side setup command using the Server Domain, Database Name, Database User, and Database Password from your CouchDB instance, or paste a setup URI generated by that command."
      )
      .addButton((button) => {
    ...
    new Setting(containerEl)
      ...
      .setDesc("Use Copy setup command, then run the copied Deno command directly on the CouchDB host, such as your server terminal, SSH session, or Docker container console. If creating a new database user or database, replace the admin_username/admin_password placeholders with an existing CouchDB admin. The command creates or verifies the database, prepares sync parameters, and prints a setup URI to paste into Use setup URI.")
      .addButton((button) => {
  ...
  private renderSchedulerSection(containerEl: SettingsContainer): void {
    ...
    this.renderNumberSetting(containerEl, "Remote check interval", "periodicSyncIntervalSec", 15, "Seconds between automatic CouchDB checks for changes from other devices. Normal desktop and mobile foreground checks are capped at 15 seconds; heavier configuration fallback scans are throttled separately.", 15);
