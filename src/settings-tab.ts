...
export class LightweightLiveSyncSettingTab extends PluginSettingTab {
  ...
  private renderSetupSection(containerEl: SettingsContainer): void {
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        this.plugin.settings.configured
          ...
          : "Connect directly to CouchDB, or paste a setup URI generated by another device or by the setup command."
      )
      .addButton((button) => {
        button.setButtonText("Connect CouchDB").setCta().onClick(async () => {
          await this.plugin.promptForDirectSetup();
    ...
    new Setting(containerEl)
      ...
      .setDesc("If in-app setup cannot create the database, use Copy setup command. Run the copied Deno command directly on the self-hosted server side where CouchDB is reachable, such as your server terminal, SSH session, or Docker container console for the CouchDB service. The command creates or verifies the database, prepares sync parameters, and prints a setup URI to paste into Use setup URI.")
      .addButton((button) => {
